...
BOOL FastDlg:: FastErase(int id, int storagetype)
{
    ...
    case TYPE_SPINAND:
        ret=XUSB_FastSPINANDErase(id, mainWnd->m_portName);
        break;
...
BOOL FastDlg:: FastBurn(int id, int storagetype)
{
    ...
    case TYPE_SPINAND:
        ret=XUSB_FastSPINANDBurn(id,mainWnd->m_portName,m_filename,&len);
        break;
...
int FastDlg:: FastVerify(int id, int storagetype)
{
    ...
    if(!m_filename.IsEmpty()) {
        ...
        case TYPE_SPINAND:
            ret=XUSB_FastSPINANDVerify(id, mainWnd->m_portName,m_filename);
            break;
...
void FastDlg::OnBnClickedRstbtn()
{
    ...
    Sleep(10);
    if(bResult == FALSE) {
        TRACE(_T("XXX RESET(%d) NUC980 failed\n"), Num);
    }
