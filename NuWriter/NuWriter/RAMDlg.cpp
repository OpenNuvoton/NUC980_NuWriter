...
BOOL CRAMDlg::InitFile(int flag)
{
    ...
    switch(flag)
    {
        case 0:
            ...
            if(m_filename.GetLength()>76)
                //tmp= m_filename.Mid(0,76);
                tmp=m_filename+_T("\r\n");
            else
                tmp= m_filename;
            GetDlgItem(IDC_SDRAM_FILENAME)->SetWindowText(tmp);
            m_fileinfo=_T(" ")+tmp;
...
void CRAMDlg::OnBnClickedSdramBrowse()
{
    ...
    this->GetDlgItem(IDC_SDRAM_FILENAME)->SetWindowText(_T(""));
    if(ret==IDCANCEL)
    {
        return;
    }
    ...
    if(m_filename.GetLength()>76)
       //filepath= m_filename.Mid(0,76);
       filepath=m_filename+_T("\r\n");

    // if(m_filename.ReverseFind('.')>0)
        // filepath=m_filename.Mid(0,m_filename.ReverseFind('.'));
    else
        filepath=m_filename;


    m_fileinfo=_T(" ")+filepath;
    //UpdateData(FALSE);

    GetDlgItem(IDC_SDRAM_FILENAME)->SetWindowText(m_fileinfo);
    mainWnd->m_inifile.SetValue(_T("SDRAM"),_T("PATH"),m_filename);
    mainWnd->m_inifile.WriteFile();
}
