...
BOOL CReadDlg::OnInitDialog()
{
    ...
    switch(type){
        ...
        case 2: /* MMC */
            ...
            break;
        case 3: /* SPI NAND */
            m_filepath.SetWindowText(mainWnd->m_inifile.GetValue(_T("SPINAND"),_T("SAVEPATH")));
            m_blocks.SetWindowText(mainWnd->m_inifile.GetValue(_T("SPINAND"),_T("nBlock")));
            m_sblocks.SetWindowText(mainWnd->m_inifile.GetValue(_T("SPINAND"),_T("sBlock")));
            break;
        default:
            _path==_T("c:");
            break;
...
void CReadDlg::OnBnClickedOk()
{
    ...
    switch(type){
        ...
        case 3: /* SPI NAND */
            m_filepath.GetWindowText(tmp);
            mainWnd->m_inifile.SetValue(_T("SPINAND"),_T("SAVEPATH"),tmp);
            m_blocks.GetWindowText(tmp);
            mainWnd->m_inifile.SetValue(_T("SPINAND"),_T("nBlock"),tmp);
            mainWnd->m_inifile.WriteFile();

            m_sblocks.GetWindowText(tmp);
            mainWnd->m_inifile.SetValue(_T("SPINAND"),_T("sBlock"),tmp);
            mainWnd->m_inifile.WriteFile();
            break;
        default:
            _path==_T("c:");
