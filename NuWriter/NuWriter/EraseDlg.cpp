...
BOOL CEraseDlg::OnInitDialog() 
{
	...
	switch(type){
		...
		case 2: /* MMC */
			...
			break;
		case 3: /* SPI NAND */
			m_blocks.SetWindowText(mainWnd->m_inifile.GetValue(_T("SPINAND"),_T("nEBlock")));	
			m_sblocks.SetWindowText(mainWnd->m_inifile.GetValue(_T("SPINAND"),_T("sEBlock")));	
			break;
		default:
			break;
	}
...
void CEraseDlg::OnBnClickedOk()
{
	...
	switch(type){
        ...
        case 3: /* SPI NAND */
			m_blocks.GetWindowText(tmp);
			mainWnd->m_inifile.SetValue(_T("SPINAND"),_T("nEBlock"),tmp);	
			mainWnd->m_inifile.WriteFile();

			m_sblocks.GetWindowText(tmp);
			mainWnd->m_inifile.SetValue(_T("SPINAND"),_T("sEBlock"),tmp);	
			mainWnd->m_inifile.WriteFile();
			break;
		default:
			break;
